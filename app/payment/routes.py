...
def get_payment(id):
    ...
    if id is not None:
        ...
        return render_template('order/client_orders.html', payments=payments)
...
def new_payment(id):
    ...
    if form.validate_on_submit():
        ...
        if payment_value > total or payment_value <= 0:
            ...
            orders = Order.query.filter_by(client_id=id, status=Status.PENDENTE).order_by(Order.date).all()
            value = payment_value
            while value > 0:
                for order in orders:
                    if value - order.cost > 0:
                        ...
                        order.remaining_cost = order.cost - value
                        value = 0
